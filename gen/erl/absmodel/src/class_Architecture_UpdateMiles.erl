-module(class_Architecture_UpdateMiles).
-include_lib("../include/abs_types.hrl").
-behaviour(object).
-export([get_val_internal/2,set_val_internal/3,init_internal/0,get_state_for_modelapi/1,implemented_interfaces/0,exported/0]).
-compile(export_all).

implemented_interfaces() -> [ <<"UpdateMilesInterface">>, <<"Object">> ].

exported() -> #{  }.

%% --- Internal state and low-level accessors

-record(state,{'class'=class_Architecture_UpdateMiles,'prometheus'=null}).
'init_internal'()->
    #state{}.

 %% architecture.abs:226
...
'm_request'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_balancer_0,V_start_0,Stack)->
 ...
 'start' => V_start_0 }),
    try
         %% architecture.abs:228--228
        cog:block_current_task_for_cpu(Cog, rationals:rdiv(( rationals:mul(( rationals:mul(m_Param_funs:f_speed_per_core(Cog,Stack),m_Param_funs:f_time_unit_to_sec(Cog,Stack))) ,5096)) ,m_Param_funs:f_update_miles_MCL(Cog,Stack)) ,[O,DC| Stack]),
        skip,
        ...
        T_1 = cog:create_task(C:get_val_internal(get(this), 'prometheus'),'m_push',[<<"latency"/utf8>>,<<"UpdateMiles"/utf8>>,( rationals:sub(maps:get('stop', get(vars)),maps:get('start', get(vars)))) ,[]],#task_info{method= <<"push"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog),
        T_1,
         %% architecture.abs:231--231
        T_2 = cog:create_task(C:get_val_internal(get(this), 'prometheus'),'m_push',[<<"completed"/utf8>>,<<"UpdateMiles"/utf8>>,1,[]],#task_info{method= <<"push"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog),
        T_2,
         %% architecture.abs:232--232
