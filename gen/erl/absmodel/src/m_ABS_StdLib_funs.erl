...
'f_deadline'(Cog=#cog{ref=CogRef},Stack)->
    (fun (V_lld_0)->case cmp:lt(V_lld_0,0) of
        true -> dataInfDuration;
        false -> { dataDuration,V_lld_0}
    end end(builtin:lowlevelDeadline(Cog))).
...
'f_subtractFromDuration'(Cog=#cog{ref=CogRef},V_d_0,V_v_0,Stack)->
    begin
        case V_d_0 of
            ...
            _ -> io:format("No match for d at abs/lang/abslang.abs:595~n"), 
            exit(dataPatternMatchFailException)
        end
    end.
